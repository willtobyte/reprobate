...
local function draw_thick_line(buf, w, h, x0, y0, x1, y1, t, rgba)
  local dx = abs(x1 - x0)
  local dy = abs(y1 - y0)
  local sx = x0 < x1 and 1 or -1
  local sy = y0 < y1 and 1 or -1
...
function Pentagram:loop()
  ...
  local rot = elapsed * 0.8
  local cy, sy = cos(rot), sin(rot)

  local cx, cy_scr = w * 0.5, h * 0.5
  local scale = self.scale

  local projx = {}
  local projy = {}
  for i = 1, 5 do
    local ang = self.pent_angles[i]
    local ux = cos(ang)
    local uy = -sin(ang)
    ...
    projx[i] = cx + xr * scale * f
    projy[i] = cy_scr + uy * scale * f
  end

  local color = char(PENTAGRAM_COLOR[1], PENTAGRAM_COLOR[2], PENTAGRAM_COLOR[3], 255)
  local t = self.thickness
  for i = 1, 5 do
    local e = self.edges[i]
    local x0 = floor(projx[e[1]])
    local y0 = floor(projy[e[1]])
    local x1 = floor(projx[e[2]])
    local y1 = floor(projy[e[2]])
    draw_thick_line(buf, w, h, x0, y0, x1, y1, t, color)
  end

  canvas.pixels = concat(buf, "", 1, self.total)
end
